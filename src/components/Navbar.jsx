...
import companyLogo from "../assets/ANFASSA.png";
import {
  FACEBOOK_URL,
  INSTAGRAM_URL,
  PHONE_NUMBER,
  ...
  return (
    <nav className="w-full h-0 fixed top-0 z-50 tracking-wider">
      <div
        className={`${
          navBar || openMobile
            ? "from-main-blue-700 to-main-blue-900"
            : "bg-white"
        } ${
          showNavbar ? "-translate-y-28" : ""
        } bg-gradient-to-br duration-300 `}
      >
          ...
          >
            <div className="flex w-full items-center justify-between">
              <div className="hidden lg:flex gap-8 items-center flex-1">
                <ul className="flex gap-4 xl:gap-7 items-center">
                  {navbarLinks.map((item, index) => (
                    <li
                      key={index}
                      className={`${
                        isActive(item, pathname)
                          ? "underline decoration-gray-700 decoration-2 underline-offset-[7px]"
                          : ""
                    ...
                    >
                      {item.link ? (
                        <a
                          href={item.link}
                          className="text-black font-light duration-300 hover:underline decoration-gray-700 decoration-2 underline-offset-[7px] py-12 border-gray-700 whitespace-nowrap"
                        >
                          {item.name}
                        </a>
                      ) : (
                        <span className="font-light cursor-default text-black duration-300 hover:underline decoration-gray-700 decoration-2 underline-offset-[10px] py-12 border-gray-700 whitespace-nowrap">
                          {item.name}
                        </span>
                      )}
                      {item.submenu && item.submenu.length > 0 && (
                        <ul
                          className={`absolute z-10 top-[52px] bg-white border border-gray-700 overflow-hidden whitespace-nowrap text-black left-0 duration-300 ${
                            hoveredIndex === index
                            ...
                            <li
                              ...
                              className={`${
                                isActive(subitem, pathname)
                                  ...
                                  : ""
                              } relative hover:bg-gray-700 duration-300 hover:text-black`}
              ...
              <a
                href="/"
                className={` ${navBar ? "w-36" : "w-40"} duration-200`}
                dangerouslySetInnerHTML={{ __html: children.props.value }}
              ></a>

              <div className="hidden lg:flex flex-1 justify-end">
                <a
                  href="mailto:anfassafotografo@hotmail.com"
      ...
      <div
        ...
        } overflow-x-hidden duration-300 ease-in-out h-screen lg:hidden absolute w-full bg-white z-50 top-0`}
        id="mobile-menu"
      >
        ...
        <ul className="px-4 pb-3 mt-5 pt-2 flex flex-col">
          {navbarLinks.map((item, index) => (
            ...
            >
              {item.link ? (
                <a
                  href={item.link}
                  className="font-light p-4 block text-black text-lg duration-300 border-gray-700 whitespace-nowrap group-last:bg-gray-700 group-last:py-4 group-last:px-8 group-last:rounded-full group-last:mt-4 group-last:text-center group-last:mx-5"
                >
                  {item.name}
                </a>
              ) : (
                <div className="font-light p-4 w-full justify-between flex cursor-pointer text-black text-lg duration-300  border-gray-700 whitespace-nowrap">
                  <p>{item.name}</p>
                  <div
              ...
              {item.submenu && item.submenu.length > 0 && (
                <ul
                  className={`z-10 ml-5 bg-black whitespace-nowrap text-black left-0 duration-500 overflow-hidden ${
                  ...
                  {item.submenu.map((subitem, subIndex) => (
                    ...
                    >
                      {subitem.link ? (
                        ...
                        <div className="font-light p-5 w-full justify-between flex cursor-pointer text-black text-lg duration-300  border-gray-700 whitespace-nowrap">
                          <p>{subitem.name}</p>
                          <div
          ...
          <li className="relative group">
            <a
              href="mailto:anfassafotografo@hotmail.com"
              className="font-light p-5 block text-black text-lg duration-300 border-gray-700 whitespace-nowrap group-last:bg-gray-700 group-last:py-4 group-last:px-8 group-last:mt-4 group-last:text-center group-last:mx-5"
            >
              Contacto
            </a>
          </li>
        ...
        <div className="p-5 px-10 text-black flex flex-col gap-3 overflow-hidden">
